...
$chosen_url = Yii::$app->urlManagerFrontend->baseUrl.'/../themes/metronic/assets/global/plugins/chosen/chosen.jquery.js';
$chosen_css = Yii::$app->urlManagerFrontend->baseUrl.'/../themes/metronic/assets/global/plugins/chosen/chosen.css';

$this->registerJsFile($ckeditor_url,['position' => \yii\web\View::POS_HEAD]);
$this->registerJsFile($chosen_url,['position' => \yii\web\View::POS_HEAD]); 
$this->registerCssFile($chosen_css,['position' => \yii\web\View::POS_HEAD]);  
...
.new-msg .new-msg-title{ margin-top:5px;}
...
<div class="project-participation-form new-adminmail" style="overflow: hidden;">
    <div class="col-xs-12 col-sm-12 new-msg"> 
        <?php $form = ActiveForm::begin(['class'=>'email-form']); ?>  
        ...
        <div id="errorNewEmail" class="help-block error-msgmail" style="display: none">Email field cannot be blank</div>
        
        
        
        <?= $form->field($model, 'subject')->textInput() ?>

        <?= $form->field($model, 'message')->textArea(['rows' => '6', 'id' => 'message', 'class' => '']) ?>          
...
<script>
  ...
  $(function () {
        ...
        $('#communique-new_email').change(function () {
            var emailtype = $('input[name="Communique[selectemail][]"]').parent('span').attr('class');
            var new_mail = $.trim($('#communique-new_email').val()); 
            if($.trim(emailtype) == "checked" && new_mail == ''){
                $('#errorNewEmail').text('Email field cannot be blank');
                $('#errorNewEmail').css('display', 'block');
                return false;
            }else if($.trim(emailtype) == "checked" && new_mail != '' && !pattern.test(new_mail)){
                $('#errorNewEmail').text('Invalid Email');
                $('#errorNewEmail').css('display', 'block');
                return false;
            }else{
                 $('#errorNewEmail').css('display', 'none');
        ...
        editor.on('change', function (evt) {            
            ...
            if(TextGrab != '' && TextGrab.match(/<\/?[^>]*>/g)){
                ...
                $('#cke_message').next().css('display','block');
                $('#cke_message').next().text('Html content is not allowed');               
                return false;
            }else if(TextGrab != '' && !TextGrab.match(/<\/?[^>]*>/g)){
                $('#cke_message').next().css('display','none');
                $('.field-message').removeClass('has-error');
                return false;
            }
            else {                
                 $('#cke_message').next().css('display','block');
                 $('#cke_message').next().text('Message cannot be blank');
                 $('.field-message').addClass('has-error');
                 return false;
            }           
    ...
    function checkErrors(){
        ...
        if($.trim(emailtype) == "checked" && new_mail != '' && !pattern.test(new_mail)){
            $('#errorNewEmail').text('Invalid Email');
            $('#errorNewEmail').css('display', 'block');
            return false;
        }
